...
export const ProjectThumbContext = React.createContext<CoordinatorAPI | null>(
  null,
);

export const ProjectThumbProvider = ({
  children,
}: {
  ...
  const api = React.useMemo<CoordinatorAPI>(
    () => ({
      ...
      notifyActivated: (id) =>
        stateRef.current.listeners.forEach((cb) => cb(id)),
      subscribe: (cb) => {
  ...
  return (
    <ProjectThumbContext.Provider value={api}>
      {children}
    </ProjectThumbContext.Provider>
  );
};
...
export const useCoordinator = (): CoordinatorAPI => {
  ...
  if (!ctx)
    throw new Error("ProjectThumb must be inside <ProjectThumbProvider>");
  return ctx;
};
